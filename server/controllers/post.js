            ...
            if(posts.length != 0){
                if('action' in reg.body){
                    if(reg.body.action == 'upvote'){
                        await postModel.updateOne({"_id": posts[0]._id}, {"upvotes": posts[0].upvotes+1})
                        console.log("upvoted " + posts[0].title)
                        res.send({"success": "true"})
                    }
                    else if(reg.body.action == 'downvote'){
                        await postModel.updateOne({"_id": posts[0]._id}, {"downvotes": posts[0].downvotes+1})
                        console.log("downvote")
                        res.send({"success": "true"})
                    }
                    else if(reg.body.action == 'toggleFavorite'){
                        await postModel.updateOne({"_id": posts[0]._id}, {"isFavorite": !posts[0].isFavorite})
